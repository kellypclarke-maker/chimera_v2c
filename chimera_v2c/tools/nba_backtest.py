"""
NBA backtest: join v2c plan log with outcomes and simulate ROI by probability buckets.
...
Usage (from repo root):
  PYTHONPATH=. python chimera_v2c/tools/nba_backtest.py --date-range 2025-10-01:2025-12-09 --league nba

Outputs:
- CSV/print table with buckets (0.05) => count, hit_rate, avg_edge, ev_model, ev_market.
- --override-buckets flags results where guardrails would block (log-only marker).
"""
...
def load_plans(date_range: Tuple[str, str], league: str, *, include_all_sides: bool = False) -> List[PlanRow]:
    ...
    for entry in data:
        ...
        if mid is None:
            continue
        by_game[(d, entry.get("matchup") or "")].append(
            PlanRow(
                date=d,
...
def simulate(plans: List[PlanRow], override_guardrails: bool) -> pd.DataFrame:
        ...
        records.append(
            {
                ...
                "stake": stake,
                "guardrail_override": override_guardrails,
                "fill_prob": fill_prob,
            }
        )
    if not records:
    ...
    grouped["guardrail_override"] = override_guardrails
    return grouped
...
def main() -> None:
    ap = argparse.ArgumentParser(description="NBA backtest on plan log vs outcomes.")
