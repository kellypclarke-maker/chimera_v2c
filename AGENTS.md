...
- **Legacy master ledger:** A historical game‑level master lives at `archive/specialist_performance/archive_old_ledgers/game_level_ml_master.csv`. It is read‑only and exists only as a reference and as the source used to backfill pre‑2025‑12‑04 daily ledgers. Do not create new scripts that write to it or run `build_game_level_ml_table.py` in normal workflows.
